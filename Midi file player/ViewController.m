...
@synthesize labelLocalSound;
@synthesize labelForLocalSoundSwitch;
@synthesize sliderTempo;
@synthesize sliderSongPosition;
@synthesize labelTempo;
@synthesize labelMIDIfile;
@synthesize labelMIDIthru1;
@synthesize labelShowSYSEX;
@synthesize labelFilterSYSEX;
@synthesize labelAutoNext;
@synthesize labelNowLen;
@synthesize labelTranspose;
@synthesize labelTranspose1;
@synthesize labelPlayBackground;
@synthesize labelChordBig;
@synthesize labelChordBigS;
@synthesize textFieldSYSEX;
@synthesize labelUseDrumSet0;
...
- (void)viewDidLoad {
    ...
    if (!initWasDone) {
        ...
        textFieldHeight = screenSize.height - textFieldSYSEX.frame.origin.y; // 182
        textFieldSYSEX.frame = CGRectMake(0, 387, 253, textFieldHeight);
    ...
    if (!initWasDone) {
        ...
        buttonMore.clipsToBounds = YES;
        buttonMIDIdevices.clipsToBounds = YES;
        ...
        buttonTrack9.layer.cornerRadius = 14;
        ...
        buttonMore.layer.cornerRadius = 7;
        buttonMIDIdevices.layer.cornerRadius = 7;
        ...
        buttonTest.layer.cornerRadius = 8;
        ...
        buttonChannel9.layer.cornerRadius = 14;
    ...
    labelLocalSound.hidden = switchLocSnd.isOn;
    labelSoundIsOff.hidden = switchLocSnd.isOn;

    sliderTempo.value = playBackTempo;
    labelTempo.text = [NSString stringWithFormat:@"Tempo: %.2f", playBackTempo];
    labelTranspose1.text = [NSString stringWithFormat:@"%d", transposeValue];
...
- (void) timerNotifyMethod {
    ...
    if (timerDivisor == 20) {
        ...
        if ([[UIDevice currentDevice] batteryState] == UIDeviceBatteryStateUnplugged) {
            ...
            if (batteryLevel < 0.1) {
                if (labelSleep.hidden) {
                    labelSleep.hidden = FALSE;
                    labelBatteryMode.hidden = FALSE;
                }
                else {
                    labelSleep.hidden = TRUE;
                    labelBatteryMode.hidden = TRUE;
...
- (void) updateBatteryLevel {
    ...
    if (batteryLevel < 0.0) {
        ...
        labelBatteryMode.text = [NSString stringWithFormat:@"Battery: ?"];
    }
    else {
        ...
        labelBatteryMode.text = [NSString stringWithFormat:@"Battery: %@", [numberFormatter stringFromNumber:levelObj]];
    }
}
...
- (void) scrollToEndOfList {
    ...
    if (buttonMoreState) {
        textFieldSYSEX.frame = CGRectMake(textFieldSYSEX.frame.origin.x, textFieldSYSEX.frame.origin.y, 320, textFieldHeight-135); // siehe auch andere Stellen im Code
    }
    else {
        textFieldSYSEX.frame = CGRectMake(textFieldSYSEX.frame.origin.x, textFieldSYSEX.frame.origin.y, 253, textFieldHeight); // siehe auch andere Stellen im Code
    }
...
- (void) timerEndMethod {
    ...
    timerforAllNotesOff =[NSTimer scheduledTimerWithTimeInterval:0.5
                                                          target:self selector:@selector(timerAllNotesOffMethod)userInfo:nil repeats:NO];
    if (!isRecording){
        [self enableButtons:TRUE];
    }
...
//
// MIDI In messages arrives here (no complete packages are received)
static void	MyMIDIReadProc(const MIDIPacketList *pktlist, void *refCon, void *connRefCon) {
    for (unsigned int i = 0; i < pktlist->numPackets; i++) {
        packet2 = pktlist->packet[i];
...
// MIDI data from MusicPlayer (always receives comlete packets, not like MIDI In)
        ...
        if (midiFilePlaying && !isPause){
            ...
            if (now >= len && !isLoop){
                ...
                timerEnd =[NSTimer scheduledTimerWithTimeInterval:0.1
                                                           target:self selector:@selector(timerEndMethod)userInfo:nil repeats:NO];
            }
            else {
...
- (void) enableButtons :(bool)enableBut {
    ...
    buttonMIDIdevices.enabled = enableBut;
...
- (IBAction)buttonSelectPressed:(id)sender {
    ...
    UIViewController *vc = [mainStoryboard instantiateViewControllerWithIdentifier:@"pickerViewID"]; // Storyboard ID
...
- (IBAction)switchLocSndValChanged:(id)sender {
    ...
    labelLocalSound.hidden = switchLocalSoundState;
    labelSoundIsOff.hidden = switchLocalSoundState;
    [self savePreferences];
    [self doRestOfLocSndChange];
...
- (void) setSmallFont {
    ...
    [self channelButtonRefreshAll];
    
}

- (void) channelButtonRefreshAll {
    if (track0state == 1) [buttonChannel0 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track0state == 2) [buttonChannel0 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel0 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    
    if (track1state == 1) [buttonChannel1 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track1state == 2) [buttonChannel1 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel1 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    
    
    if (track2state == 1) [buttonChannel2 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track2state == 2) [buttonChannel2 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel2 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    
    if (track3state == 1) [buttonChannel3 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track3state == 2) [buttonChannel3 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel3 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    
    if (track4state == 1) [buttonChannel4 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track4state == 2) [buttonChannel4 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel4 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    
    if (track5state == 1) [buttonChannel5 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track5state == 2) [buttonChannel5 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel5 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    
    if (track6state == 1) [buttonChannel6 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track6state == 2) [buttonChannel6 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel6 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    
    if (track7state == 1) [buttonChannel7 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track7state == 2) [buttonChannel7 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel7 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    
    if (track8state == 1) [buttonChannel8 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track8state == 2) [buttonChannel8 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel8 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    
    if (track9state == 1) [buttonChannel9 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track9state == 2) [buttonChannel9 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel9 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    
    if (track10state == 1) [buttonChannel10 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track10state == 2) [buttonChannel10 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel10 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    
    if (track11state == 1) [buttonChannel11 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track11state == 2) [buttonChannel11 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel11 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    
    if (track12state == 1) [buttonChannel12 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track12state == 2) [buttonChannel12 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel12 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    
    if (track13state == 1) [buttonChannel13 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track13state == 2) [buttonChannel13 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel13 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    
    if (track14state == 1) [buttonChannel14 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track14state == 2) [buttonChannel14 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel14 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
    
    if (track15state == 1) [buttonChannel15 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track15state == 2) [buttonChannel15 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel15 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
...
- (IBAction)buttonChannel0pressed:(id)sender {
    channelSelection = 0;
    [self channelChange];
    [buttonChannel0.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track0state == 1) [buttonChannel0 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track0state == 2) [buttonChannel0 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel0 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel1pressed:(id)sender {
    channelSelection = 1;
    [self channelChange];
    [buttonChannel1.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track1state == 1) [buttonChannel1 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track1state == 2) [buttonChannel1 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel1 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel2pressed:(id)sender {
    channelSelection = 2;
    [self channelChange];
    [buttonChannel2.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track2state == 1) [buttonChannel2 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track2state == 2) [buttonChannel2 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel2 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel3pressed:(id)sender {
    channelSelection = 3;
    [self channelChange];
    [buttonChannel3.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track3state == 1) [buttonChannel3 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track3state == 2) [buttonChannel3 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel3 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel4pressed:(id)sender {
    channelSelection = 4;
    [self channelChange];
    [buttonChannel4.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track4state == 1) [buttonChannel4 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track4state == 2) [buttonChannel4 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel4 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel5pressed:(id)sender {
    channelSelection = 5;
    [self channelChange];
    [buttonChannel5.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track5state == 1) [buttonChannel5 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track5state == 2) [buttonChannel5 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel5 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel6pressed:(id)sender {
    channelSelection = 6;
    [self channelChange];
    [buttonChannel6.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track6state == 1) [buttonChannel6 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track6state == 2) [buttonChannel6 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel6 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel7pressed:(id)sender {
    channelSelection = 7;
    [self channelChange];
    [buttonChannel7.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track7state == 1) [buttonChannel7 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track7state == 2) [buttonChannel7 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel7 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel8pressed:(id)sender {
    channelSelection = 8;
    [self channelChange];
    [buttonChannel8.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track8state == 1) [buttonChannel8 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track8state == 2) [buttonChannel8 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel8 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel9pressed:(id)sender {
    channelSelection = 9;
    [self channelChange];
    [buttonChannel9.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track9state == 1) [buttonChannel9 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track9state == 2) [buttonChannel9 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel9 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel10pressed:(id)sender {
    channelSelection = 10;
    [self channelChange];
    [buttonChannel10.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track10state == 1) [buttonChannel10 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track10state == 2) [buttonChannel10 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel10 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel11pressed:(id)sender {
    channelSelection = 11;
    [self channelChange];
    [buttonChannel11.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track11state == 1) [buttonChannel11 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track11state == 2) [buttonChannel11 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel11 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel12pressed:(id)sender {
    channelSelection = 12;
    [self channelChange];
    [buttonChannel12.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track12state == 1) [buttonChannel12 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track12state == 2) [buttonChannel12 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel12 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel13pressed:(id)sender {
    channelSelection = 13;
    [self channelChange];
    [buttonChannel13.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track13state == 1) [buttonChannel13 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track13state == 2) [buttonChannel13 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel13 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel14pressed:(id)sender {
    channelSelection = 14;
    [self channelChange];
    [buttonChannel14.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track14state == 1) [buttonChannel14 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track14state == 2) [buttonChannel14 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel14 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
- (IBAction)buttonChannel15pressed:(id)sender {
    channelSelection = 15;
    [self channelChange];
    [buttonChannel15.titleLabel setFont:[UIFont boldSystemFontOfSize:25]];
    if (track15state == 1) [buttonChannel15 setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
    else if (track15state == 2) [buttonChannel15 setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    else [buttonChannel15 setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
}
...
- (void) chordDetection {
    ...
    if (chordType[chordPointer] != nil) {
        labelChordBigText = [NSString stringWithFormat:@"%@%@", notesForText[lowestNoteChord], chordType[chordPointer]];
        labelChordBig.text = labelChordBigText;
        labelChordBigS.text = labelChordBigText;
        starPointer = bufferForChord[outpointBut][0];
        [self setStar];
        labelChordBig.hidden = FALSE;
        labelChordBigS.hidden = FALSE;
        labelChordChannel.hidden = FALSE;
        labelChordBigState = FALSE;
    }
    else {
        chordPointer = 0;
        if (last12notes[chordChannel][0][1] > 0) chordPointer = chordPointer + 1;
        ...
        if (chordTypeBase[chordPointer] != nil) {
            labelChordBigText = [NSString stringWithFormat:@"%@", chordTypeBase[chordPointer]];
            labelChordBig.text = labelChordBigText;
            labelChordBigS.text = labelChordBigText;
            starPointer = bufferForChord[outpointBut][0];
            [self setStar];
...
- (void) setStar {
    int offsteSettings = 0;
    if (isSettingView) offsteSettings = 608;
    else offsteSettings = 0;

    switch (starPointer) {
        case 0:
            labelChordChannel.frame = CGRectMake(24, offsteSettings+176, 13, 26);
            break;
        case 1:
            labelChordChannel.frame = CGRectMake(24+40, offsteSettings+176, 13, 26);
            break;
        case 2:
            labelChordChannel.frame = CGRectMake(24+80, offsteSettings+176, 13, 26);
            break;
        case 3:
            labelChordChannel.frame = CGRectMake(24+120, offsteSettings+176, 13, 26);
            break;
        case 4:
            labelChordChannel.frame = CGRectMake(24+160-1, offsteSettings+176, 13, 26);
            break;
        case 5:
            labelChordChannel.frame = CGRectMake(24+200-2, offsteSettings+176, 13, 26);
            break;
        case 6:
            labelChordChannel.frame = CGRectMake(24+240-2, offsteSettings+176, 13, 26);
            break;
        case 7:
            labelChordChannel.frame = CGRectMake(24+380-3, offsteSettings+176, 13, 26);
            break;
        case 8:
            labelChordChannel.frame = CGRectMake(24, offsteSettings+176+38, 13, 26);
            break;
        case 9:
            labelChordChannel.frame = CGRectMake(24+40, offsteSettings+176+38, 13, 26);
            break;
        case 10:
            labelChordChannel.frame = CGRectMake(24+80, offsteSettings+176+38, 13, 26);
            break;
        case 11:
            labelChordChannel.frame = CGRectMake(24+120, offsteSettings+176+38, 13, 26);
            break;
        case 12:
            labelChordChannel.frame = CGRectMake(24+160-1, offsteSettings+176+38, 13, 26);
            break;
        case 13:
            labelChordChannel.frame = CGRectMake(24+200-2, offsteSettings+176+38, 13, 26);
            break;
        case 14:
            labelChordChannel.frame = CGRectMake(24+240-2, offsteSettings+176+38, 13, 26);
            break;
        case 15:
            labelChordChannel.frame = CGRectMake(24+280-3, offsteSettings+176+38, 13, 26);
            break;
        default:
            break;
    }
...
- (IBAction)buttonMorePressed:(id)sender {
    buttonMoreState = !buttonMoreState;
    [self moreLessTreating];
}

- (void) moreLessTreating {
    labelForLocalSoundSwitch.hidden = !buttonMoreState;
    switchLocSnd.hidden = !buttonMoreState;
    labelPlayBackground.hidden = !buttonMoreState;
    switchBackgroundPlay.hidden = !buttonMoreState;
    labelMIDIthru1.hidden = !buttonMoreState;
    switchMIDIthru.hidden = !buttonMoreState;
    labelShowSYSEX.hidden = !buttonMoreState;
    switchShowSYSEX.hidden = !buttonMoreState;
    labelFilterSYSEX.hidden = !buttonMoreState;
    switchFilterSYSEX.hidden = !buttonMoreState;
    switchAutoNext.hidden = !buttonMoreState;
    labelAutoNext.hidden = !buttonMoreState;
    switchDrumSet0.hidden = !buttonMoreState;
    labelUseDrumSet0.hidden = !buttonMoreState;
    
    if (buttonMoreState) {
        textFieldSYSEX.frame = CGRectMake(0, 525, 320, textFieldHeight-135); // siehe auch andere Stellen im Code
        textFieldSYSEX.hidden = FALSE;
    }
    else {
        textFieldSYSEX.frame = CGRectMake(0, 387, 253, textFieldHeight); // siehe auch andere Stellen im Code
        textFieldSYSEX.hidden = buttonMoreState;
    }
    
    if (buttonMoreState) {
        [buttonMore setTitle:@"Show Messages" forState:UIControlStateNormal];
    }
    else {
        [buttonMore setTitle:@"Show Options" forState:UIControlStateNormal];
    }
}
...
- (IBAction)buttonTopPressed:(id)sender {
    ...
    frame = labelMIDIfile.frame;
    frame.origin.y = 46;
    labelMIDIfile.frame = frame;
    
    frame = buttonPlay.frame;
    frame.origin.y = 120;
    buttonPlay.frame = frame;
    
    frame = buttonPause.frame;
    frame.origin.y = 120;
    buttonPause.frame = frame;
    
    frame = buttonStop.frame;
    frame.origin.y = 120;
    buttonStop.frame = frame;
    
    frame = buttonRecording.frame;
    frame.origin.y = 120;
    buttonRecording.frame = frame;
    
    frame = labelNowLen.frame;
    frame.origin.y = 256;
    labelNowLen.frame = frame;
    
    frame = sliderSongPosition.frame;
    frame.origin.y = 281;
    sliderSongPosition.frame = frame;
    
    frame = labelTempo.frame;
    frame.origin.y = 329;
    labelTempo.frame = frame;
    
    frame = sliderTempo.frame;
    frame.origin.y = 325;
    sliderTempo.frame = frame;
    
    frame = buttonLoop.frame;
    frame.origin.y = 277;
    buttonLoop.frame = frame;
    
    frame = buttonStartPoint.frame;
    frame.origin.y = 277;
    buttonStartPoint.frame = frame;
    
    frame = buttonEndPoint.frame;
    frame.origin.y = 304;
    buttonEndPoint.frame = frame;
    
    frame = labelStartPoint.frame;
    frame.origin.y = 306;
    labelStartPoint.frame = frame;

    frame = labelEndPoint.frame;
    frame.origin.y = 306;
    labelEndPoint.frame = frame;
    
    frame = labelMIDI_0.frame;
    frame.origin.y = 164;
    labelMIDI_0.frame = frame;
    frame = labelMIDI_1.frame;
    frame.origin.y = 164;
    labelMIDI_1.frame = frame;
    frame = labelMIDI_2.frame;
    frame.origin.y = 164;
    labelMIDI_2.frame = frame;
    frame = labelMIDI_3.frame;
    frame.origin.y = 164;
    labelMIDI_3.frame = frame;
    frame = labelMIDI_4.frame;
    frame.origin.y = 164;
    labelMIDI_4.frame = frame;
    frame = labelMIDI_5.frame;
    frame.origin.y = 164;
    labelMIDI_5.frame = frame;
    frame = labelMIDI_6.frame;
    frame.origin.y = 164;
    labelMIDI_6.frame = frame;
    frame = labelMIDI_7.frame;
    frame.origin.y = 164;
    labelMIDI_7.frame = frame;
    frame = labelMIDI_8.frame;
    frame.origin.y = 243;
    labelMIDI_8.frame = frame;
    frame = labelMIDI_9.frame;
    frame.origin.y = 243;
    labelMIDI_9.frame = frame;
    frame = labelMIDI_10.frame;
    frame.origin.y = 243;
    labelMIDI_10.frame = frame;
    frame = labelMIDI_11.frame;
    frame.origin.y = 243;
    labelMIDI_11.frame = frame;
    frame = labelMIDI_12.frame;
    frame.origin.y = 243;
    labelMIDI_12.frame = frame;
    frame = labelMIDI_13.frame;
    frame.origin.y = 243;
    labelMIDI_13.frame = frame;
    frame = labelMIDI_14.frame;
    frame.origin.y = 243;
    labelMIDI_14.frame = frame;
    frame = labelMIDI_15.frame;
    frame.origin.y = 243;
    labelMIDI_15.frame = frame;
    
}

- (IBAction)buttonSettingsPressed:(id)sender {
    textFieldSYSEX.hidden = TRUE;
    isSettingView = TRUE;
...
- (IBAction)buttonMIDIdevicesPressed:(id)sender {
    viewToGetBack = @"ViewControllerID";
}
